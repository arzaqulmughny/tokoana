...
class StockInHistoryController extends Controller
{
    ...
    {
        $data = StockIn::with('items', 'user', 'supplier')
            ->when($request->search, function ($query, string $searchQuery) {
                $query->whereRelation('supplier', 'name', 'LIKE', '%'. $searchQuery .'%')->orWhere('note', 'LIKE', '%'. $searchQuery .'%');
            })
        ...
        return Inertia::render('History/In', [
            'user' => Auth::user(),
    ...
    {
        $data = StockIn::all()->find($id)->load('user','items', 'supplier');
